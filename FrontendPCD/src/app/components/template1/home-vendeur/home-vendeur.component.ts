...
export class HomeVendeurComponent implements OnInit{
  ...
  constructor(private service:ProduitService,private templateService:TemplateContentService, private http: HttpClient,private router: Router,private vendeurservice:VendeurService){}
 ...
 this.getTemplateByVendeur(); this.getAllProducts(this.vendeur.id);
});}
getAllProducts(i:number){this.service.getProductsByVendeur(i).subscribe(data=>{this.produitF=data;} )}

  ngOnInit(){
    if(this.vendeurservice.build==true){ this.BuildTemplate();}
  ...
  BuildTemplate() {
    ...
    const options: MySweetAlertOptions = {
      title: 'Création de votre template est en cours ! ',
      html: 'Il sera prêt dans <b></b> millisecondes!',
      timer: 2000,
